...
def handle_command(command, channel):
    ...
    if command.startswith(EXAMPLE_COMMAND):
        ...
        try:
            roto_id = data["ROTOWIREID"].loc[player_name.lower()] #finds location of player name in dataframe and fetches rotowire ID
            if(len(roto_id.shape) == 0):
                roto_id = int(roto_id)
            else:
                roto_id = int(roto_id.iloc[0])
